...
const ambientLight = new THREE.AmbientLight(0x404040, 0.3);  // Reduced from 1.0
scene.add(ambientLight);
const directionalLight = new THREE.DirectionalLight(0xffffff, 0.4);  // Reduced from 1.5
directionalLight.position.set(1, 1, 1);
scene.add(directionalLight);
const pointLight = new THREE.PointLight(0xffffff, 0.3);  // Reduced from 1.2
pointLight.position.set(0, 0, 2);
scene.add(pointLight);
const hemisphereLight = new THREE.HemisphereLight(0x87CEEB, 0x8B4513, 0.15);  // Reduced from 0.6
...
let boundingBox;  // Declare boundingBox here
loader.load(
    '/public/models/frontwall3d.glb',  // File name as specified
    function (gltf) {
        ...
        gltfScene.traverse((child) => {
            if (child.isMesh) {
                ...
                // Special case: Change color of "floore" to black
                if (child.name === "floore") {
...
generateBtn.addEventListener('click', function() {
    if (modelLoaded) {
        ...
        if (boundingBox) {
            ...
            controls.maxDistance = maxDim * 0.9;  // Allow zooming out to twice the max dimension
        }
